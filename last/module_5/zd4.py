...
def findK(v, l, r, segment_tree, k): #функция проходит по всему дереву, начиная с первой вершины, переходя в левого или правого потомка, в зависимости от того, где находиться нужный ноль
    if k > segment_tree[v]: #если k-го нуля не существует, возвращаем -1
        return -1
    if r - l == 1:
        return r 
    m = (r+l)//2
    if segment_tree[2*v+1] >= k: #если значение левого потомка больши или равно k, то переходим туда, т.к. там точно есть k ноль
        return findK(2*v+1, l, m, segment_tree, k)
    else: #иначе переходим в правого потомка
