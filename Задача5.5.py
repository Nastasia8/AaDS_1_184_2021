...
def build(v, l, r, segment_tree, nums):
    ...
    segment_tree[v] = math.gcd(segment_tree[2 * v + 1], segment_tree[2 * v + 2])
def update(v, l, r, segment_tree, indx, value):
    ...
    segment_tree[v] = math.gcd(segment_tree[2 * v + 1], segment_tree[2 * v + 2])
def get(v, l, r, segment_tree, ql, qr):
    if ql <= l and qr >= r:
        return segment_tree[v]
    if ql >= r or qr <= l:
        return 0
    m = (r + l) // 2
    st_l = get(2 * v + 1, l, m, segment_tree, ql, qr)
    st_r = get(2 * v + 2, m, r, segment_tree, ql, qr)
...
def main():
    ...
    while q != 0:
        ...
        if type_q == 's':
            arr.append(get(0, 0, n, segment_tree, int(l) - 1, int(r)))
        else:
