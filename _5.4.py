...
def getSum(v,l,r,segment_tree,ql,qr):  #Функция для нахождения суммы 
    if ql<=l and qr>=r:
...
def findK(v, l, r, segment_tree, k):   #Функция для поиска нулей 
    if k > segment_tree[v]:
        return -1
    if r - l == 1:
        return r
...
def main():
    ...
    while q !=0:
        ...
        if sum >= k and l == 1:    #Выводим К-ый слева 0 элемент 
