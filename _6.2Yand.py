def shift_up(index, heap):     #Функция сдвига вверх
    while heap[index] > heap[(index-1)//2] and (index-1)//2 >= 0:
        heap[index], heap[(index-1)//2] = heap[(index-1)//2], heap[index]
        index = (index-1)//2
    return index+1

def shift_down(index, heap):     #Функция сдвига вниз
...
def add(item, heap):                      #Добавление нового элемента в кучу
    heap.append(item)
    return shift_up(len(heap)-1, heap)

def direct_extract(index, heap):      #удаляем из очереди элемент
    val = heap[len(heap)-1]
...
def extract(heap):              # Функция извлечения максимального элемента из кучи
...
def get_max(heap):        #Функция поиска максимального элемента кучи 
    return heap[0]
...
def main():
    ...
    for i in range(m):                        #если элемент с индексом не существовал, то в массив добавляем -1, если же был, то добавляем значение данного элемента.
        data = input().split()
        if int(data[0]) == 1:
            if not heap:
                result.append(-1)
