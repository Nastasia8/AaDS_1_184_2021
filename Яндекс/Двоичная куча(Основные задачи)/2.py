...
def add(item, heap): #+добавляем
    heap.append(item)
    return shift_up(len(heap)-1, heap)

def direct_extract(ind, heap):#-основное извлечение
    v = heap[len(heap)-1]
    heap[ind-1], heap[len(heap)-1] = heap[len(heap)-1], heap[ind-1]
    ind = heap.pop()
    if v < ind:
        shift_down(ind-1, heap)
    elif v > ind:
        shift_up(ind-1, heap)
    return ind
...
main()
# Не дай бог задачи из 6 модуля на экзамен...))
